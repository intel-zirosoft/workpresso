...
## 🧑‍💻 Part 8: 상세 UI 정렬 및 삭제 기능 보강

**목표:** 문서 상세 경험을 작성 경험과 통일하고, 문서 삭제 정책을 안전하게 추가

- [ ] **Frontend (UI)**: 문서 상세 레이아웃을 문서 작성 오버레이와 동일한 정보 구조, 액션 배치, 헤더 톤으로 재구성
- [ ] **Frontend (UX)**: 상세 화면에서 읽기/검토 흐름이 끊기지 않도록 상태, 결재선, 공람, 본문 영역의 우선순위를 재정렬
- [ ] **Frontend (Bugfix)**: 문서 작성 우상단 닫기 버튼 2개가 동시에 보이는 문제를 수정하고 외곽 닫기 버튼만 남기기
- [ ] **Backend (API)**: `DELETE /api/documents/[id]` 엔드포인트 추가
- [ ] **Backend (Policy)**: 문서 작성자 본인 문서만 삭제 가능하도록 권한 검증 추가
- [ ] **Backend (Policy)**: `APPROVED` 문서는 관리자만 삭제 가능하도록 상태 + 역할 기반 제한 추가
- [ ] **Backend (Data)**: 문서 삭제 시 `deleted_at` 기반 소프트 삭제 처리 및 목록/상세 조회에서 제외 규칙 확인
- [ ] **Frontend (UI)**: 작성자에게만 삭제 액션 노출, `APPROVED` 비관리자 문서에서는 삭제 버튼 숨김 또는 비활성 처리
- [ ] **Frontend (UX)**: 삭제 전 확인 모달/경고 문구 추가 및 삭제 후 목록/상세 캐시 동기화 처리
