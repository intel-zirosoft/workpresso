...
import { Trash2, CheckSquare, Square, X, History, ArrowLeft, FileAudio, Mic2 } from 'lucide-react';
import { listMeetingLogs, deleteMeetingLogs } from '@/features/pod-d/services/meetingLogService';
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between mb-2 px-2">
            ...
            {!isDeleteMode ? (
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={handleSelectAll} className="h-8 text-xs rounded-full">
          ...
          {isDeleteMode && (
            <div className="flex items-center justify-between bg-destructive/5 p-3 rounded-lg border border-destructive/10 animate-in slide-in-from-top-1 duration-200">
              ...
              <Button 
                ...
                className="h-8 px-4 rounded-full text-xs shadow-soft"
              >
                선택 삭제 실행
              </Button>
            </div>
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              logs.map((log) => (
                <Card 
                  key={log.id} 
                  onClick={isDeleteMode ? (e) => toggleSelect(e, log.id) : () => setSelectedLog(log)}
                  className={cn(
                    "p-5 bg-surface hover:bg-white transition-all cursor-pointer border-none shadow-soft flex items-center gap-4 group relative",
                    isDeleteMode && selectedIds.has(log.id) && "ring-2 ring-primary/30 bg-white"
                  )}
                >
                  {isDeleteMode && (
