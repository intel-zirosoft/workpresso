...
export default async function SystemSettingsPage() {
  ...
  try {
    ...
    if (ext)
      llmConfig = { config: ext.config as any, is_active: ext.is_active };
  } catch (e) {}
    ...
    await upsertExtension(
      "system_llm",
      {
        ...
        chatModel: normalizeModelId(
          formData.get("chatModel") as string,
          "chat",
        ),
        embeddingModel: normalizeModelId(
          formData.get("embeddingModel") as string,
          "embedding",
        ),
        meetingRefineModel: normalizeModelId(
          formData.get("meetingRefineModel") as string,
          "meetingRefine",
        ),
  ...
  return (
    <div className="animate-in fade-in duration-700 space-y-12">
      {/* Dashboard Header */}
      <div>
        <h1 className="text-3xl font-headings font-bold text-text">
          시스템 설정 및 관리
        </h1>
        <p className="text-sm text-text-muted mt-1 font-body">
          워크프레소 코어 인프라와 외부 API 엔진을 제어합니다.{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div
            key={idx}
            className="bg-surface border border-background p-6 rounded-3xl shadow-soft flex items-center gap-4"
          >
          ...
          <APIKeyForm
            ...
            fields={[
              ...
              },
              {
                ...
                type: "text",
                defaultValue:
                  llmConfig.config?.chatModel || "openai/gpt-4o-mini",
              },
              {
                name: "embeddingModel",
                label: "임베딩 모델",
                type: "text",
                defaultValue:
                  llmConfig.config?.embeddingModel ||
                  "openai/text-embedding-3-small",
              },
              {
                name: "meetingRefineModel",
                label: "회의록 정제 모델",
                type: "text",
                defaultValue:
                  llmConfig.config?.meetingRefineModel ||
              ...
              },
              {
                ...
                type: "text",
                defaultValue:
                  llmConfig.config?.sttModel || "openai/gpt-4o-audio-preview",
              },
            ]}
            action={updateLLM}
      ...
      <div className="p-6 bg-primary/5 rounded-3xl border border-primary/10 flex items-start gap-4">
        ...
        <p className="text-xs text-text-muted leading-relaxed">
          <span className="font-bold text-primary">안내:</span> 시스템 설정
          변경은 전역 인프라에 즉시 반영됩니다. OpenRouter API 키는 서버 DB에
          암호화 저장되며, 저장된 키가 없을 때만 환경변수 fallback을 사용합니다.
        </p>
      </div>
    </div>
  );
