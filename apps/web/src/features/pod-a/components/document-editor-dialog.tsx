...
export function DocumentEditorDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        hideCloseButton
        className="flex h-[calc(100vh-1rem)] max-h-[calc(100vh-1rem)] w-[calc(100vw-1rem)] max-w-7xl flex-row gap-0 overflow-hidden border-none bg-surface p-0 shadow-2xl sm:h-[95vh]"
      >
        {/* Left Sidebar */}
        <aside className="w-[280px] bg-background/60 border-r border-background/60 flex flex-col p-8 shrink-0">
          <div className="mb-12">
            <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-primary mb-1">Steps</h2>
            <p className="text-[13px] text-text-muted font-medium tracking-tight">Document Progress</p>
          </div>
              ...
              return (
                ...
                >
                  {isCompleted ? (
                    ...
                    <StepIcon className={cn("h-5 w-5", isActive ? "text-primary" : "text-text-muted")} />
                  )}
          ...
          <footer className="mt-auto pt-8">
            <button
              type="button"
              className="group flex items-center justify-center w-8 h-8 rounded-full border border-background text-text-muted hover:text-primary hover:border-primary hover:bg-primary/5 transition-all focus:outline-none"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-surface relative">
          <button
            type="button"
            onClick={onCancel}
            className="absolute top-6 right-8 p-2 rounded-full hover:bg-background text-text-muted transition-colors z-50"
          >
            <X className="h-6 w-6" />
          </button>
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-16 pb-24 px-12">
              <div className="max-w-5xl mx-auto">
                <header className="mb-12">
                  ...
                  <h1 className="text-4xl font-headings font-black text-text tracking-tight mb-3">
                    {currentStep === "template" ? "원하는 템플릿을 선택하세요" : 
                     currentStep === "content" ? "문서의 제목과 내용을 입력하세요" : 
                     "결재선과 공람 대상을 설정하세요"}
                  </h1>
                  <p className="text-[15px] font-medium text-text-muted leading-relaxed max-w-2xl">
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            isSelected
                              ...
                              : "border-background/60 bg-surface hover:border-primary/40 hover:shadow-lg hover:-translate-y-1"
                          )}
                        >
                          ...
                          <div className={cn(
                            ...
                            isSelected
                              ? "bg-surface/80 text-text-muted border-primary/20"
                              : "bg-background/50 text-text-muted border-background/60 group-hover:bg-surface"
                          )}>
                             <div className="space-y-4">
                               <div className={cn("font-bold text-[14px]", isSelected ? "text-primary" : "text-text-muted")}>
                                 ## {template.id === 'expense' ? '지출' : (template.id === 'leave' ? '휴가' : '요청')} 개요
                               </div>
                               <div className="space-y-2 px-0.5">
                                 {template.id === 'general' && (
                                   <>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-primary font-bold">제목:</span> <span className="bg-background h-3 w-16 rounded-sm" /></div>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-primary font-bold">목적:</span> <span className="bg-background h-3 w-32 rounded-sm" /></div>
                                   </>
                                 )}
                                 {template.id === 'expense' && (
                                   <>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-text-muted font-bold">지출 항목:</span> <span className="bg-background h-3 w-16 rounded-sm" /></div>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-text-muted font-bold">지출 목적:</span> <span className="bg-background h-3 w-32 rounded-sm" /></div>
                                   </>
                                 )}
                                 {template.id === 'leave' && (
                                   <>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-text-muted font-bold">휴가 종류:</span> <span className="bg-background h-3 w-16 rounded-sm" /></div>
                                     <div className="flex items-center gap-2 text-[12.5px] opacity-80">- <span className="text-text-muted font-bold">신청 기간:</span> <span className="bg-background h-3 w-40 rounded-sm" /></div>
                                   </>
                                 )}
                                 {template.id === 'skip' && (
                ...
                {currentStep === "content" ? (
                  <div className="space-y-8">
                    <div className="space-y-2">
                      <label htmlFor="document-title" className="text-[15px] font-bold text-text">제목</label>
                      <div className="relative">
                        <Input
                          ...
                          className="h-12 rounded-xl border-background/60 bg-background/50 focus:bg-surface pr-16 text-lg font-medium"
                          disabled={isDisabled}
                        />
                        <div className="pointer-events-none absolute inset-y-0 right-4 flex items-center text-xs text-text-muted font-bold">
                    ...
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-background/80 rounded-xl border border-background/60">
                        ...
                        <div className="w-[1px] h-4 bg-background mx-1" />
                        ...
                        <div className="ml-auto flex items-center gap-1.5">
                          ...
                          <Button 
                            ...
                            variant="ghost" 
                            size="sm"
                            className="rounded-lg h-8 px-3 text-text-muted"
                      ...
                      <div className="relative group">
                        {!isPreviewVisible ? (
                          <textarea
                            ...
                            className="min-h-[500px] w-full resize-none rounded-2xl border-background/60 bg-background/30 p-8 font-body text-[15px] leading-relaxed text-text outline-none transition focus:bg-surface focus:ring-4 focus:ring-primary/5 disabled:opacity-50"
                            disabled={isDisabled}
                          />
                        ) : (
                          <div className="min-h-[500px] rounded-2xl border border-background/60 bg-surface p-8">
                            <PreviewPanel title={editorState.title} content={editorState.content} />
                          </div>
                        )}
                        <div className="absolute bottom-4 right-6 text-[11px] font-black text-text/30 tracking-wider">
                  ...
                  <div className="grid gap-8 xl:grid-cols-[1fr_0.8fr]">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-bold text-text">결재선 구성</h3>
                        <Button type="button" variant="outline" size="sm" className="rounded-lg border-background font-bold" onClick={onAddApprovalStep} disabled={isMutating}>
                      ...
                      <div className="space-y-3">
                        {editorState.approvalSteps.map((step, index) => (
                          <div key={step.localId} className="group relative flex items-center gap-4 bg-background/70 p-4 rounded-2xl border border-background/60 transition-all hover:bg-surface hover:shadow-md hover:border-primary/20">
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-surface text-xs font-black text-primary border border-background/60">
                              {index + 1}
                            </div>
                            <div className="flex-1 space-y-1">
                               <input
                                 className="w-full bg-transparent font-bold text-text outline-none focus:text-primary transition-colors"
                                 value={step.stepLabel}
                                 onChange={(e) => onUpdateApprovalStep(step.localId, "stepLabel", e.target.value)}
                                 placeholder={`단계 ${index + 1}`}
                               />
                               <select
                                 className="w-full bg-transparent text-sm text-text-muted outline-none appearance-none cursor-pointer"
                    ...
                    <div className="space-y-6">
                      <h3 className="text-lg font-bold text-text">공람 대상</h3>
                      <div className="bg-background/50 rounded-2xl border border-background/60 p-6 flex flex-wrap gap-2">
            ...
            {showExpandedLayer && (
              <div className="absolute inset-0 z-50 flex flex-col bg-surface animate-in fade-in zoom-in-95 duration-300">
                <div className="border-b border-background/60 px-6 py-4 flex items-center justify-between">
                  <div className="font-bold text-text">집중 보기 모드</div>
            ...
            <div className="h-20 border-t border-background/60 bg-surface flex items-center px-12 shrink-0 z-10">
              <div className="flex-1 flex items-center gap-3">
                <div className="flex flex-col gap-1.5">
                  <div className="text-[10px] font-black text-text tracking-wider">
                    STEP {currentStepIndex + 1} OF 4
                  </div>
                  <div className="w-32 h-[3px] bg-background rounded-full overflow-hidden">
                    <div 
              ...
              <div className="flex items-center gap-4">
                ...
                {previousStep && (
                  <Button
                    type="button"
                    variant="outline"
                    className="rounded-xl font-bold px-7 border-background"
