...
function MessageBubble({ message }: { message: ChatterMessageSummary }) {
  return (
    ...
    >
      ...
      <div className="max-w-[92%] space-y-2 sm:max-w-[88%] md:max-w-[72%]">
        <div
          className={cn(
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(300px,360px)] xl:grid-cols-[minmax(0,1.2fr)_420px]">
        <header className="rounded-[32px] bg-[linear-gradient(135deg,rgba(127,161,195,0.18),rgba(242,193,141,0.16),rgba(255,255,255,0.92))] px-6 py-6 shadow-soft md:px-8">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-headings font-bold tracking-tight text-text md:text-4xl">
                채터
              </h1>
              <p className="max-w-3xl text-sm leading-6 text-text/80 md:text-base">
                대화, 멤버, 업무 링크를 한 화면에서 정리해 팀 커뮤니케이션의 흐름을
                더 빠르게 파악할 수 있도록 구성했습니다.
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2">
          ...
          <MetricCard
            ...
            tone="border-primary/10 bg-primary/10"
          />
          <MetricCard
            label="현재 멤버"
            value={String(channelDetail?.members.length ?? activeChannelSummary?.memberCount ?? 0)}
            tone="border-secondary/10 bg-secondary/20"
          />
          <MetricCard
            label="공유 링크"
            value={String(channelDetail?.sharedItems.length ?? 0)}
            tone="border-success/10 bg-success/20"
          />
        </div>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)_320px]">
        <Card className="overflow-hidden border-transparent bg-surface shadow-soft lg:sticky lg:top-6 lg:flex lg:max-h-[calc(100vh-3rem)] lg:flex-col">
        ...
        <Card className="flex min-h-[640px] flex-col overflow-hidden border-transparent bg-surface shadow-soft sm:min-h-[720px] xl:min-h-[calc(100vh-3rem)]">
            ...
            {activeChannelSummary ? (
              <div className="space-y-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  ...
                  <Button
                    variant="outline"
                    className="rounded-pill border-transparent bg-background px-4 shadow-soft"
                    disabled
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  <ContextChip
                    ...
                    label="멤버"
                    value={String(
                      channelDetail?.members.length ?? activeChannelSummary.memberCount
                    )}
                  />
                  <ContextChip
                    icon={Sparkles}
                    label="활성"
                    value={`${activeMembersCount}명 업무 가능`}
                  />
                  <ContextChip
                    icon={Clock3}
                    label="최근 활동"
                    value={formatChannelActivity(activeChannelSummary.lastActivityAt)}
                  />
                </div>
          ...
          <div className="border-t border-background/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(253,251,247,0.96))] px-3 py-4 sm:px-4 md:px-6">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <span className="inline-flex items-center rounded-pill bg-background px-3 py-1.5 text-xs font-bold text-text">
                제안 {quickShareCount}개
              </span>
            </div>

            <div className="mb-4 flex flex-wrap items-center gap-2">
              {shareTargets.documents.slice(0, 2).map((target) => (
                <button
                  className="max-w-full rounded-pill bg-background px-3 py-1.5 text-left text-xs font-bold text-text transition-colors hover:bg-primary/10"
                  key={target.id}
              ...
              {shareTargets.schedules.slice(0, 2).map((target) => (
                <button
                  className="max-w-full rounded-pill bg-background px-3 py-1.5 text-left text-xs font-bold text-text transition-colors hover:bg-primary/10"
                  key={target.id}
                  onClick={() => handleShareTarget(target)}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:col-span-2 xl:col-span-1 xl:grid-cols-1">
          <Card className="border-transparent bg-surface p-5 shadow-soft">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <span className="rounded-pill bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary">
                활성 {activeMembersCount}명
              </span>
            </div>
          ...
          <Card className="border-transparent bg-surface p-5 shadow-soft">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <span className="rounded-pill bg-background px-3 py-1.5 text-xs font-bold text-text">
                {channelDetail?.sharedItems.length ?? 0}개
              </span>
            </div>
            <div className="space-y-3">
